...
function ValidateSubmit()
{
	...
	else
	{
		...
		{
			if (document.uploadForm1.userfile.value.length < 1)
			{
				alert('Please select a file to convert. This is obviously important because else you wouldn\'t be using this application, now would you?');
				document.uploadForm1.userfile.focus();
				return false;
			}
			else
			{
			var re2 = new RegExp('[:\]');
		    if(document.uploadForm1.userfile.value.match(re2))
				return true;				
			else
				alert('Please select a valid file to convert. This is obviously important because else you wouldn\'t be using this application, now would you?');
				document.uploadForm1.userfile.focus();
				return false;
			}
		} 
...
<hr noshade color="#C0C0C0" size="1">
		...
		<p>Note that if you can't get this to work for you there are some other 
		alternatives available. Word 2007 features a nifty feature in that you 
		can download and install a
		<a target="_blank" href="http://www.microsoft.com/downloads/details.aspx?FamilyId=4D951911-3E7E-4AE6-B059-A2E79ED87041&displaylang=en">
		Save As PDF</a> add-in allowing you to save directly to PDF (or XPS for 
		that matter).</p>
